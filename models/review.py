...
class Review(Base):
    ...
    stores = relationship('Store', secondary='review_store')
...
class ReviewEntitySummary(Base):
    __tablename__ = 'review_entity_summary'
    __table_args__ = {'comment': 'Review aggregates'}

    primary_id = Column(BIGINT(20), primary_key=True, comment='Summary review entity ID')
    entity_pk_value = Column(BIGINT(20), nullable=False, server_default=text("0"), comment='Product ID')
    entity_type = Column(SMALLINT(6), nullable=False, server_default=text("0"), comment='Entity type ID')
    reviews_count = Column(SMALLINT(6), nullable=False, server_default=text("0"), comment='Qty of reviews')
    rating_summary = Column(SMALLINT(6), nullable=False, server_default=text("0"), comment='Summarized rating')
...
t_review_store = Table(
    'review_store', metadata,
    Column('review_id', ForeignKey('review.review_id', ondelete='CASCADE'), primary_key=True, nullable=False, comment='Review ID'),
...
class ReviewDetail(Base):
    ...
    __table_args__ = {'comment': 'Review detail information'}

    detail_id = Column(BIGINT(20), primary_key=True, comment='Review detail ID')
    review_id = Column(ForeignKey('review.review_id', ondelete='CASCADE'), nullable=False, index=True, server_default=text("0"), comment='Review ID')
    store_id = Column(ForeignKey('store.store_id', ondelete='SET NULL'), index=True, server_default=text("0"), comment='Store ID')
    title = Column(String(255), nullable=False, comment='Title')
    detail = Column(Text, nullable=False, comment='Detail description')
    nickname = Column(String(128), nullable=False, comment='User nickname')
