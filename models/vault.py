...
class VaultPaymentToken(Base):
    __tablename__ = 'vault_payment_token'
    __table_args__ = (
        Index('VAULT_PAYMENT_TOKEN_PAYMENT_METHOD_CODE_CSTR_ID_GATEWAY_TOKEN', 'payment_method_code', 'customer_id', 'gateway_token', unique=True),
        {'comment': 'Vault tokens of payment'}
    )

    entity_id = Column(INTEGER(10), primary_key=True, comment='Entity ID')
    customer_id = Column(ForeignKey('customer_entity.entity_id', ondelete='CASCADE'), index=True, comment='Customer ID')
    public_hash = Column(String(128), nullable=False, unique=True, comment='Hash code for using on frontend')
    payment_method_code = Column(String(128), nullable=False, comment='Payment method code')
    type = Column(String(128), nullable=False, comment='Type')
    created_at = Column(TIMESTAMP, nullable=False, server_default=text("current_timestamp()"), comment='Created At')
    expires_at = Column(TIMESTAMP, comment='Expires At')
    gateway_token = Column(String(255), nullable=False, comment='Gateway Token')
    details = Column(Text, comment='Details')
    is_active = Column(TINYINT(1), nullable=False, server_default=text("1"))
    is_visible = Column(TINYINT(1), nullable=False, server_default=text("1"))
...
class SalesOrderPayment(Base):
    ...
    payment_tokens = relationship('VaultPaymentToken', secondary='vault_payment_token_order_payment_link')


t_vault_payment_token_order_payment_link = Table(
    'vault_payment_token_order_payment_link', metadata,
    Column('order_payment_id', ForeignKey('sales_order_payment.entity_id', ondelete='CASCADE'), primary_key=True, nullable=False, comment='Order payment ID'),
    Column('payment_token_id', ForeignKey('vault_payment_token.entity_id', ondelete='CASCADE'), primary_key=True, nullable=False, index=True, comment='Payment token ID'),
