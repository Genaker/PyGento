...
class GoogleoptimizerCode(Base):
    __tablename__ = 'googleoptimizer_code'
    __table_args__ = (
        Index('GOOGLEOPTIMIZER_CODE_STORE_ID_ENTITY_ID_ENTITY_TYPE', 'store_id', 'entity_id', 'entity_type', unique=True),
        {'comment': 'Google Experiment code'}
    )

    code_id = Column(INTEGER(10), primary_key=True, comment='Google experiment code ID')
    entity_id = Column(INTEGER(10), nullable=False, comment='Optimized entity ID product ID or catalog ID')
    entity_type = Column(String(50), comment='Optimized entity type')
    store_id = Column(ForeignKey('store.store_id', ondelete='CASCADE'), nullable=False, comment='Store ID')
    experiment_script = Column(Text, comment='Google experiment script')
